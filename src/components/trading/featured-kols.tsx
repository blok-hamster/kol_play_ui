                ...
                {leaderboard.map((item, index) => {
                    ...
                    return (
                        <div
                            ...
                            className="group relative bg-card/50 backdrop-blur-sm border border-border/50 hover:border-primary/50 transition-all duration-300 rounded-xl p-4 cursor-pointer hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-1 overflow-hidden"
                        >
                            {/* Rank Badge */}
                            <div className={`absolute top-0 right-0 p-2 rounded-bl-xl text-[10px] font-bold tracking-wider px-3 py-1
                            ...
                            <div className="flex flex-col h-full justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="relative">
                                        ...
                                        {index < 3 && (
                                            <div className="absolute -bottom-1 -right-1 bg-background rounded-full border border-border p-0.5">
                                                <Trophy className={`w-3 h-3 ${index === 0 ? 'text-yellow-500' :
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {metadata?.socialLinks?.twitter && (
                                            <p className="text-[10px] text-muted-foreground truncate hover:text-blue-400 transition-colors">
                                ...
                                <div className="grid grid-cols-2 gap-2 pt-3 border-t border-border/50">
                                    <div className="space-y-0.5">
                                        ...
                                        <p className="text-sm font-semibold">{(item.stats.winRate * 100).toFixed(1)}%</p>
                                    </div>
                                    <div className="space-y-0.5">
                                        <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                                            <Users className="w-3 h-3" /> Volume
                                        </p>
                                        <p className="text-sm font-semibold">{formatNumber(item.stats.totalVolume, 1)}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
